...
# プロンプトの簡潔化（重要！）
# 具体的な事業所種別はユーザー入力から判断させるか、別途ユーザーに設定させる
# 各心理療法のアプローチも、行動指針としてAIが理解していれば良いので、全てをプロンプトに記載する必要はない
...
ユーザーの思考を深掘り、強みと行動を促すオープンな質問を含めてください。
回答の最後に、建設的な質問を必ず含めてください。
専門用語は避け、分かりやすい言葉で説明してください。
AIの限界を認識し、必要に応じ専門家への相談を促してください。
簡潔さを最優先し、無駄なトークン消費を避けてください。
"""
# 例として、さらに短くするなら：
# MANAGEMENT_SUPPORT_SYSTEM_PROMPT = """
# あなたは「役職者お悩みサポート」として、障害福祉施設の管理職の悩みに寄り添い、具体的かつ実践的なアドバイスを提供します。
# 傾聴と共感を忘れず、ユーザーの強みを引き出し、前向きな行動を促す質問をしてください。
# 回答は簡潔にまとめ、最後に問いかけを含めてください。必要に応じて専門機関への相談を促します。
# """
...
def handle_message(event):
    ...
    if user_id not in user_sessions or user_sessions[user_id]['last_request_date'] != current_date:
        ...
        personalized_initial_message = INITIAL_MESSAGE # ユーザー名を含めないシンプルな初期メッセージにする
